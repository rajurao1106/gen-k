  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="rounded-2xl border border-[#d8b36a]/20 bg-[#0a1529]/80 backdrop-blur p-6 shadow-xl shadow-black/30">
        <h2 className="font-display text-xl font-semibold text-[#f5efe6] mb-1">महादशा</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {BIRTH_FIELDS.map((field) => (
            <div key={field.key}>
              <label className="block text-xs font-medium text-[#f4d7a7] mb-1.5">{field.label}</label>
              <input
                type={field.type}
                name={field.key}
      ...
      {loading && (
        <div className="rounded-2xl border border-[#d8b36a]/20 bg-[#0a1529]/60 backdrop-blur p-10 text-center">
          ...
          <p className="text-sm text-[#afbdd7]">दशा क्रम तैयार किया जा रहा है…</p>
        </div>
      )}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="rounded-xl border border-[#d8b36a]/50 bg-[#d8b36a]/10 p-4">
              <p className="text-[10px] uppercase tracking-wide text-[#d8b36a] mb-1">वर्तमान महादशा</p>
              <p className="font-display text-lg font-semibold text-[#f5efe6]">{result.currentMahadasha.planet}</p>
            ...
            <div className="rounded-xl border border-[#d8b36a]/20 bg-[#111d31]/70 p-4">
              <p className="text-[10px] uppercase tracking-wide text-[#afbdd7] mb-1">वर्तमान अंतर्दशा</p>
              <p className="font-display text-lg font-semibold text-[#f5efe6]">{result.currentAntardasha.planet}</p>
              <p className="text-xs text-[#afbdd7] mt-1">
          ...
          <div className="rounded-2xl border border-[#d8b36a]/20 bg-[#0a1529]/60 backdrop-blur p-6 shadow-xl shadow-black/30">
            <h3 className="font-display text-lg font-semibold text-[#d8b36a] mb-4">दशा क्रम (Timeline)</h3>
                    ...
                    <div className="flex flex-col items-center">
                      <span
                        className={`h-3 w-3 rounded-full shrink-0 mt-1 ${
                          isCurrent ? "bg-[#d8b36a]" : "bg-[#8ea1c2]"
                        }`}
                      />
                      {i < result.timeline.length - 1 && <span className="w-px flex-1 bg-[#d8b36a]/20 my-1" />}
                    </div>
                    <div className={`pb-4 ${isCurrent ? "text-[#d8b36a]" : "text-[#e3cbb0]"}`}>
          ...
          {/* Yearly outlook */}
          <div className="rounded-2xl border border-[#d8b36a]/20 bg-[#0a1529]/60 backdrop-blur p-6 shadow-xl shadow-black/30">
            <h3 className="font-display text-lg font-semibold text-[#d8b36a] mb-4">आने वाले वर्षों का दृष्टिकोण</h3>
            <div className="space-y-3">
              {result.yearlyOutlook.map((y, i) => (
                <div key={i} className="rounded-lg border border-[#d8b36a]/15 bg-[#111d31]/60 p-4">
                  <div className="flex items-center justify-between mb-1.5">
                    <p className="font-display text-base font-semibold text-[#f5efe6]">{y.year}</p>
